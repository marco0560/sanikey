...
def test_extract_text_uses_ocrmypdf_when_pymupdf_is_unavailable(
    ...
    def fake_run(command, **_kwargs):
        assert command[command.index("--user-words") + 1] == str(
            documents_module.TESSERACT_MEDICAL_USER_WORDS
        )
...
def test_extract_text_forces_ocr_for_scanned_pdf_with_native_text(
    tmp_path: Path,
    monkeypatch,
) -> None:
    """Verify forced OCR replaces sufficient text only for scanned PDFs.
    ...
    person = _person(tmp_path)
    person.source_documents.mkdir(parents=True)
    path = person.source_documents / "20260102 Report.pdf"
    path.write_bytes(b"%PDF synthetic")
    document = scan_documents(person)[0]
    native = documents_module.ExtractedText(
        document_id=document.document_id,
        text="This is enough extracted text from a scanned PDF document.",
    )
    ocr = documents_module.ExtractedText(
        document_id=document.document_id,
        text="text regenerated by OCR",
    )

    monkeypatch.setattr(
        documents_module, "_extract_pdf_text_with_pymupdf", lambda _: native
    )
    monkeypatch.setattr(documents_module, "_is_scanned_pdf", lambda _: True)
    monkeypatch.setattr(
        documents_module, "_extract_pdf_text_with_ocrmypdf", lambda _: ocr
    )

    extracted = extract_text(document, force_pdf_ocr=True)

    assert extracted == ocr


def test_scanned_pdf_classification_requires_a_full_page_image(tmp_path: Path) -> None:
    """Verify a digital page prevents scanned-PDF classification.
    ...
    scanned_page.insert_image(scanned_page.rect, filename=image_path)
    scanned.save(scanned_path)
    scanned.close()

    hybrid_path = tmp_path / "hybrid.pdf"
    hybrid = fitz.open()
    hybrid_page = hybrid.new_page(width=600, height=800)
    hybrid_page.insert_image(hybrid_page.rect, filename=image_path)
    digital_page = hybrid.new_page(width=600, height=800)
    digital_page.insert_text((72, 72), "Native digital text")
    hybrid.save(hybrid_path)
    hybrid.close()

    assert documents_module._is_scanned_pdf(scanned_path)
    assert not documents_module._is_scanned_pdf(hybrid_path)
